...
[pandas](https://github.com/pandas-dev/pandas) : Python 数据分析模块
...
`说明` : 本文是 pandas 官网 [10 Minutes to pandas](http://pandas.pydata.org/pandas-docs/stable/10min.html) 的翻译。
...
-   更多章节请查看[Cookbook](http://pandas.pydata.org/pandas-docs/stable/cookbook.html#cookbook)

创建对象
--------

详情请查看[数据结构介绍](http://pandas.pydata.org/pandas-docs/stable/dsintro.html#dsintro)

1.通过传入一个列表来创建[Series](http://pandas.pydata.org/pandas-docs/stable/generated/pandas.Series.html#pandas.Series) ，pandas 会创建默认的整形指标:
```python
    ...
    dtype: float64
```
2.通过传递数字数组、时间索引、列标签来创建[DataFrame](http://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.html#pandas.DataFrame):
```python
...
```
4.查看各列的[dtypes](http://pandas.pydata.org/pandas-docs/stable/basics.html#basics-dtypes):
```python
    >>> df2.dtypes
        A           float64
        B    datetime64[ns]
...
详情请查看[基本功能](http://pandas.pydata.org/pandas-docs/stable/basics.html#basics)
...
```
5.按[axis](http://stackoverflow.com/questions/22149584/what-does-axis-in-pandas-mean)排序:
```python
...
详情请查看: [Indexing and Selecting Data](http://pandas.pydata.org/pandas-docs/stable/indexing.html#indexing)
...
详情请查看[通过位置选择](http://pandas.pydata.org/pandas-docs/stable/indexing.html#indexing-integer)
...
```
3.通过[isin()](http://pandas.pydata.org/pandas-docs/stable/generated/pandas.Series.isin.html#pandas.Series.isin)过滤数据:
```python
    >>> df2 = df.copy()
...
pandas 用 np.nan 代表缺失数据，详情请查看 [Missing Data section](http://pandas.pydata.org/pandas-docs/stable/missing_data.html#missing-data)
...
详情请查看 [Basic section on Binary Ops](http://pandas.pydata.org/pandas-docs/stable/basics.html#basics-binop)
...
详情请查看
[直方图和离散化](http://pandas.pydata.org/pandas-docs/stable/basics.html#basics-discretization)
...
```python
        ...
        >>> s.value_counts()
            ...
            dtype: int64
```
pandas 默认配置了一些字符串处理方法，可以方便的操作元素，如下所示:(详情请查看[Vectorized String Methods](http://pandas.pydata.org/pandas-docs/stable/text.html#text-string-methods))
...
\- 连接
pandas 提供了大量的方法，能轻松的对 Series，DataFrame 和 Panel 执行合并操作。详情请查看[Merging section](http://pandas.pydata.org/pandas-docs/stable/merging.html#merging)
...
```python
    ...
    >>> pd.concat(pieces)
        ...
        9  0.764893  0.612208 -1.022504 -2.032126
```
\- Join 类似 SQL 的合并操作，详情请查看 [Database style joining](http://pandas.pydata.org/pandas-docs/stable/merging.html#merging-join)
...
```python
    ...
    >>> pd.merge(left, right, on='key')
        ...
        1  bar     2     5
```
-   追加，详情请查看[Appending](http://pandas.pydata.org/pandas-docs/stable/merging.html#merging-concatenation):
```python
...
详情请查看 [Grouping section](http://pandas.pydata.org/pandas-docs/stable/groupby.html#groupby)
...
详情请查看 [Hierarchical Indexing](http://pandas.pydata.org/pandas-docs/stable/advanced.html#advanced-hierarchical)和[Reshaping](http://pandas.pydata.org/pandas-docs/stable/reshaping.html#reshaping-stacking)
...
```python
    ...
    >>> df2
        ...
        baz   one     0.241927  1.130320
              two    -0.261823  2.463877
```
`注`:pd.MultiIndex.from\_tuples 将包含多个 list 的元组转换为复杂索引
...
```python
    ...
    >>> stacked.unstack(0)
        ...
        two    A -0.825565 -0.261823
               B -0.880527  2.463877
```
`透视表` 详情请查看 [Pivot Tables](http://pandas.pydata.org/pandas-docs/stable/reshaping.html#reshaping-pivot)
...
pandas 可以简单高效的进行重新采样通过频率转换(例如:将秒级数据转换成五分钟为单位的数据)。这常见与金融应用中，但是不限于此。详情请查看
...
```
`注`:to\_period()默认频率为 M，to\_period 和 to\_timestamp 可以相互转换
...
从 0.15 版以后，pandas 可以造 DataFrame 中包含分类数据，详情请查看[分类介绍](http://pandas.pydata.org/pandas-docs/stable/categorical.html#categorical)
和 [API 文档](http://pandas.pydata.org/pandas-docs/stable/api.html#api-categorical):
```python
...
详情请查看[Plotting](http://pandas.pydata.org/pandas-docs/stable/visualization.html#visualization):
```python
...
```
[图一](http://pandas.pydata.org/pandas-docs/stable/_images/series_plot_basic.png)
...
```
[图二](http://pandas.pydata.org/pandas-docs/stable/_images/frame_plot_basic.png)

获取数据 写入导出
-----------------

\- CSV 1.
[写入 csv 文件](http://pandas.pydata.org/pandas-docs/stable/io.html#io-store-in-csv):
```python
    >>> df.to_csv('foo.csv')
```
2.  [读取 csv 文件](http://pandas.pydata.org/pandas-docs/stable/io.html#io-read-csv-table):
```python
        >>> pd.read_csv('foo.csv')
                 Unnamed: 0          A          B         C          D
...
```
请查看[Comparisons](http://pandas.pydata.org/pandas-docs/stable/basics.html#basics-compare)来处理异常，也可以查看[Gotchas](http://pandas.pydata.org/pandas-docs/stable/gotchas.html#gotchas)
...
[pandas 行转列、列转行、以及一行生成多行 - 古明地盆 - 博客园](https://www.cnblogs.com/traditional/p/11967360.html)
