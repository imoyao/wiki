...
Pickling是如此的重要，以至于它不仅仅有自己的模块（ `pickle` ），还有自己的协议和魔法方法。首先，我们先来简要的介绍一下如何 pickle 已存在的对象类型（如果你已经知道了，大可跳过这部分内容）。
...
- Python 3 中 string 和 unicode 的区别不复存在，因此 `__unicode__` 被取消了， `__bytes__` 加入进来（与Python 2.7 中的 `__str__` 和 `__unicode__` 行为类似），用于新的创建字节数组的内建方法。
