...
:   基础：`sqlalchemy.sql.visitors.Visitable`
    ...
    所有SQL数据类型的最终基类。plainplainplainplainplainplainplainplainplainplainplainplainplain
...
:   mixin 标记类型为支持索引操作，如数组或 JSON 结构。
    ...
    版本1.1.0中的新功能plainplainplainplainplainplainplainplainplainplainplainplain
...
:   基础：[`sqlalchemy.types.TypeEngine`](#sqlalchemy.types.TypeEngine "sqlalchemy.types.TypeEngine")
    ...
    一个未知的类型。plainplainplainplainplainplainplainplainplain
...
:   基础：[`sqlalchemy.types.TypeDecorator`](custom_types.html#sqlalchemy.types.TypeDecorator "sqlalchemy.types.TypeDecorator")
    ...
    一种包装类型，可根据使用的方言在各种实现中进行选择。plainplainplainplainplainplainplainplainplainplainplainplain
