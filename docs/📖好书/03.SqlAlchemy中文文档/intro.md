...
当`pip`可用时，可以从 Pypi 下载并在一个步骤中安装该分发：
    ...
    pip install SQLAlchemyplainplainplainplain
...
为了安装最新的**prerelease**版本，比如`1.1.0b1`，pip 要求使用`--pre`标志：
    ...
    pip install --pre SQLAlchemyplainplainplainplainplain
...
否则，您可以使用`setup.py`脚本从分发安装：
    ...
    python setup.py installplainplain
...
要运行构建/安装而不尝试编译 C 扩展，可以指定`DISABLE_SQLALCHEMY_CEXT`环境变量。对于这种情况的用例要么是针对特殊的测试环境，要么是通常的“重建”机制无法解决的罕见情况下的兼容性/构建问题：
    ...
    export DISABLE_SQLALCHEMY_CEXT=1; python setup.py installplainplainplainplainplainplainplain
