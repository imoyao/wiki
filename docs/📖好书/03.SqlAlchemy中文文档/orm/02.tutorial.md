...
> 注意
>
> [`match()`](core_sqlelement.html#sqlalchemy.sql.operators.ColumnOperators.match "sqlalchemy.sql.operators.ColumnOperators.match")使用数据库特定的`MATCH`或`CONTAINS`函数；其行为将随后端而变化，并且在某些后端（如SQLite）上不可用。
