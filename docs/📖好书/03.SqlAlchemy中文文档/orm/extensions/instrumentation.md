...
:   属性，在存在于映射类上时选择自定义检测。
    ...
    允许类指定一个稍微或非常不同的技术来跟踪对映射的属性和集合所做的更改。plainplainplain
...
:   用户定义的类工具扩展。
    ...
    [`InstrumentationManager`](#sqlalchemy.ext.instrumentation.InstrumentationManager "sqlalchemy.ext.instrumentation.InstrumentationManager")plain
