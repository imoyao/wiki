...
而 SDS 的 API 都会以处理二进制的方式来处理存放在`buf`数组里的数据，不会对里面的数据做任何的限制。SDS使用`len`属性的值来判断字符串是否结束，而不是空字符
...
链表的定义如下，`adlist.h/list`
    ...
    typedef struct list {plainplain
        // 链表头节点
        listNode *head;
        // 链表尾节点
        listNode *tail;
...
哈希表结构定义，`dict.h/dictht`
    ...
    typedef struct dictht {plainplain
        // 哈希表数组
        dictEntry **table;
        // 哈希表大小
        unsigned long size;
...
哈希表是由数组`table`组成，`table`中每个元素都是指向`dict.h/dictEntry`结构的指针，哈希表节点的定义如下
    ...
    typedef struct dictEntry {plain
        // 键
        void *key;
        // 值
        union {
...
`type`属性和`privdata`属性是针对不同类型的键值对，用于创建多类型的字典，`type`是指向`dictType`结构的指针，`privdata`则保存需要传给类型特定函数的可选参数，关于`dictType`结构和类型特定函数可以看下面代码
    ...
    typedef struct dictType {plainplain
        // 计算哈希值的行数
...
Redis 的跳跃表实现是由`redis.h/zskiplistNode`和`redis.h/zskiplist`（3.2 版本之后 redis.h 改为了 server.h）两个结构定义，`zskiplistNode`定义跳跃表的节点，`zskiplist`保存跳跃表节点的相关信息
    ...
    /* ZSETs use a specialized version of Skiplists */plainplainplainplainplain
...
整数集合的定义为`inset.h/inset`
    ...
    typedef struct intset {plainplain
        // 编码方式
        uint32_t encoding;
        // 集合包含的元素数量
        uint32_t length;
