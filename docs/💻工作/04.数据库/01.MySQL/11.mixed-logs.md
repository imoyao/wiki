...
| 日志类型     | 存储位置                                                          | 物理 / 逻辑特性                                          | 核心作用                          | 写入时机                                            | 是否可关闭     |
|----------|---------------------------------------------------------------|----------------------------------------------------|-------------------------------|-------------------------------------------------|-----------|
| undo log | 系统表空间（`ibdata1`）或独立表空间（`*.ibd` 文件，取决于 `innodb_undo_tablespaces` 配置） | 物理 + 逻辑：记录数据页物理变更，但内容是逻辑操作（如 DELETE 反向为 INSERT）    | 回滚事务、MVCC（多版本并发控制）            | 事务执行过程中实时写入                                     | 不可关闭      |
| redo log | 磁盘文件（默认位于 `datadir` 下的 `ib_logfile0`、`ib_logfile1` 等循环文件）         | 物理：记录物理页变更（如 “将页 X 的偏移量 Y 位置的值改为 Z”）               | 崩溃恢复（保证事务持久性）、提高写入性能（先写日志后刷盘） | 事务提交前（`fsync` 取决于 `innodb_flush_log_at_trx_commit`） | 不可关闭      |
| binlog   | 磁盘文件（默认位于 `datadir` 下的 `mysql-bin.xxxxxx` 文件，通过 `log_bin` 启用）      | 逻辑：记录 SQL 语句或行变更（如 `INSERT INTO users VALUES (...)`） | 主从复制、数据恢复（基于时间点）、审计           | 事务提交后（组提交优化，批量写入）                               | 可关闭（默认关闭） |
...
实现这一机制就是 **undo log（回滚日志），它保证了事务的 [ACID 特性](<https://xiaolincoding.com/mysql/transaction/mvcc.html>)中的原子性（Atomicity）**。
...
这两个隔离级别实现是通过「事务的 Read View 里的字段」和「记录中的两个隐藏列（trx\_id 和 roll\_pointer）」的比对，如果不满足可见行，就会顺着 undo log 版本链里找到满足其可见性的记录，从而控制并发事务访问同一个记录时的行为，这就叫 MVCC（多版本并发控制）。具体的实现可以看我这篇文章：[事务隔离级别是怎么实现的？](<https://xiaolincoding.com/mysql/transaction/mvcc.html>)
...
* [MySQL 的 undo 日志---MVCC 前置知识 - 大队长 11 - 博客园](https://www.cnblogs.com/duizhangz/archive/2022/06/01/16333565.html)
