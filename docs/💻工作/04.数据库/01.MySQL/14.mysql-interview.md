...
![引擎特性对比](https://pic.imgdb.cn/item/6514fa97c458853aef99399f.jpg)
...
能够使用 `SHOW ENGINES` 语句查看你的 MySql Server 所支持的存储引擎
...
[警惕 InnoDB 和 MyISAM 创建 Hash 索引陷阱 - cynchanpin - 博客园](https://www.cnblogs.com/cynchanpin/p/7003390.html)
...
:::note
对于varchar字段，一般情况下，使用普通索引比前缀索引更合适。普通索引可以完整地对字段进行索引，而不仅仅是字段的前几个字符。这样可以提高索引的选择性和查询的准确性。

前缀索引适用于以下情况：

1. 字段的长度较长，但是只有前几个字符对查询有用。例如，一个较长的URL字段，但是只需要根据前几个字符进行查询。
2. 字段的选择性较低，即字段的取值不够唯一。在这种情况下，使用前缀索引可以减少索引的大小，提高查询性能。
