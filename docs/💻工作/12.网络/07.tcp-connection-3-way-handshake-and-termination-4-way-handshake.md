...
:::note
- **防止已失效的连接请求报文突然又传送到服务器：** 如果只有两次握手，假设一个延迟很久的旧 `SYN` 到达服务器，服务器会直接建立连接并发送 `SYN+ACK`，然后等待客户端数据。但客户端可能早已放弃了这个连接，不会发送数据，导致服务器资源浪费（半开连接）。三次握手要求客户端必须再次确认，如果客户端不回应 `ACK`，服务器最终会超时关闭这个无效连接。
